...
export function Notifications() {
  ...
  const getIconAndColor = (type: string) => {
    switch (type) {
      case 'alert':
        return { icon: AlertCircle, bgColor: 'bg-red-50', borderColor: 'border-red-200', iconColor: 'text-red-600', iconBg: 'bg-red-100' };
      case 'warning':
        return { icon: AlertTriangle, bgColor: 'bg-yellow-50', borderColor: 'border-yellow-200', iconColor: 'text-yellow-600', iconBg: 'bg-yellow-100' };
      case 'success':
        return { icon: CheckCircle2, bgColor: 'bg-green-50', borderColor: 'border-green-200', iconColor: 'text-green-600', iconBg: 'bg-green-100' };
      case 'info':
      default:
        return { icon: Info, bgColor: 'bg-blue-50', borderColor: 'border-blue-200', iconColor: 'text-blue-600', iconBg: 'bg-blue-100' };
    }
  };
  ...
  return (
    <div className="p-4 lg:p-8 max-w-7xl mx-auto">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900 mb-2">Powiadomienia</h1>
          ...
          {notificationsList.length === 0 ? (
            ...
            <div className="space-y-3">
              {notificationsList.map((notification) => {
                const { icon: Icon, bgColor, borderColor, iconColor, iconBg } = getIconAndColor(notification.type);
                return (
                  <div
                    key={notification.id}
                    className={`rounded-xl p-4 border transition-all ${notification.read ? 'bg-white opacity-60' : bgColor} ${borderColor}`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        {!notification.read && (
                          <Button onClick={() => markAsRead(notification.id)} variant="ghost" size="sm" className="h-8 w-8 p-0">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200 h-fit">
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between gap-3">
              <div>
                <Label htmlFor="notifications-toggle" className="font-medium text-slate-900">
                  Powiadomienia
                </Label>
                <p className="text-sm text-slate-600 mt-1">Włącz lub wyłącz wszystkie</p>
              </div>
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                <Label htmlFor="device-status-toggle" className="font-medium text-slate-900">
                  Status urządzeń
                </Label>
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                <Label htmlFor="automation-toggle" className="font-medium text-slate-900">
                  Alerty automatyzacji
                </Label>
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                <Label htmlFor="energy-toggle" className="font-medium text-slate-900">
                  Raporty energii
                </Label>
